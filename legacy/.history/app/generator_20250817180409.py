...
ФОРМАТ: Краткий фактологический ответ без лишних слов."""
    ...
    def validate_answers_batch(self, questions: List[str], generated_answers: List[str], reference_answers: List[str]) -> List[ValidationScore]:
        ...
        validation_prompt = (
            ...
            "ВАЖНО: Верни JSON в точно таком формате:\n"
            '{"results": [{"index": 0, "score": 0.8}, {"index": 1, "score": 0.9}]}\n\n'
            "Вопросы и ответы:\n"
        )
        ...
        messages = [
                    SystemMessage(
                    content="Ты - эксперт по валидации. Отвечай ТОЛЬКО валидным JSON в указанном формате. Никаких дополнительных слов или объяснений."
                    ), 
