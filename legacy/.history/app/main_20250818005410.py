...
# Параметры чанкинга
DEFAULT_CHUNK_SIZE = 1200  # Оптимальный размер чанка в символах
...
def print_results_summary(outputs: List[Dict]) -> None:
    """
    Выводит красивое резюме результатов в консоль.
    ...
    Формат вывода:
        ...
        - Основной ответ (без списка источников)
    ...
    for i, result in enumerate(outputs, 1):
        ...
        # Извлекаем основной ответ (убираем блок источников)
        ...
        # Показываем оценку качества с цветовой индикацией
        if 'validation' in result:
            score = result['validation']['score']
            if score >= 0.8:
                score_emoji = "🟢"  # Отличное качество
            elif score >= 0.6:
                score_emoji = "🟡"  # Хорошее качество
            else:
                score_emoji = "🔴"  # Требует улучшения
    ...
    if outputs:
        ...
        avg_score = sum(
            result.get('validation', {}).get('score', 0) 
        ...
        print(f"   • Среднее чанков на вопрос: {total_sources/total_questions:.1f}")
...
def load_and_print_results(results_filename: str) -> None:
    """
    Загружает и выводит результаты из сохранённого JSON файла.
    
    Args:
        results_filename: Имя файла с результатами в папке results/
    ...
    try:
        ...
        logger.info(f"📂 Загружены результаты из: {results_filename}")
        print_results_summary(results)
        
    except FileNotFoundError:
        logger.error(f"❌ Файл не найден: {results_filename}")
        available_files = [f for f in os.listdir(RESULTS_DIR) if f.endswith('.json')]
        if available_files:
            logger.info(f"Доступные файлы: {', '.join(available_files[-5:])}")  # Последние 5
